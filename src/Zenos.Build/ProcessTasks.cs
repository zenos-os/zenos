...
public static class ProcessTasks
{
    ...
    /// </summary>
    private static bool IsWsl { get; } = GetIsWsl();

    private static bool GetIsWsl()
    {
        System.Console.WriteLine("EnvironmentInfo.IsLinux: " + EnvironmentInfo.IsLinux);
        ...
        try
        {
            ...
            System.Console.WriteLine("/proc/version: " + version);
            return version.ContainsOrdinalIgnoreCase("Microsoft");
        }
        catch (IOException ex)
        {
            System.Console.WriteLine("/proc/version: " + ex);
            return false;
        }
    }
...
#if NETCORE
        if (EnvironmentInfo.IsUnix && !IsWsl && toolPath.EndsWithOrdinalIgnoreCase(".exe"))
